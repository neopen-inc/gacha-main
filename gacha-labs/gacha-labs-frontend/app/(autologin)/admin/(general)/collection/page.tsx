...
import { StatusTag } from "@gacha-labs-app/components/admin/status-tag";
import { SimpleBreadcrumb } from "@gacha-labs-app/components/common/simple-breadcrumb";
import { createCollection, fetchCollections, fetchCategories, removeCollection, updateCollection, prepareCreateCollection, prepareUpdateCollection, prepareRemoveCollection, fetchCollectionInitializeStatus, clearUpdateCollection, clearCreateCollection, clearRemoveCollection } from "@gacha-labs-app/domain/oripa/action";
...
import { CollectionCardCount } from "@gacha-labs-app/domain/oripa/dto";
import { Collection } from "@gacha-labs-app/domain/types";
    ...
    <dialog.FormDialog open={updateCollectionOperation.status === 'confirm' || createCollectionOperation.status === 'confirm'}
      ...
      <div className="space-y-4">
        ...
        <Controller
            ...
            control={collectionForm.control}
            render={({ field }) => <>
            <FormControlLabel control={<Switch checked={field.value} onChange={field.onChange} size="small" />} label="1日一回限定" />
            <br />
            </>}
          />
        <Controller
