...
import { User, Collection, CollectionProgress, Gacha } from "@gacha-expo-app/domain/types";
import { createGacha, fetchCollectionById, fetchCollectionProgress, fetchSceneByGrade } from "@gacha-expo-app/domain/oripa/action";
  ...
  return <div className="w-full flex flex-row justify-center">
    ...
    <div style={{ height: `${dimensions.height}px`, width: `${getOripaWindowSize()}px`  }} >
      <div className="block sticky top-0 w-full h-screen -z-50">
        <img src={fetchCollectionByIdOperation.payload?.background}  style={{ ...(scrollPosition > 100 ? {filter: `blur(${scrollPosition / 20}px)`} : {}) }} className="w-full" />
      </div>
      <div className="z-10">
