...
import { Collection, CollectionProgress } from "@gacha-expo-app/domain/types";
import { fetchCategories, fetchCollectionProgress, fetchCollections } from "@gacha-expo-app/domain/oripa/action";
import { checkin, fetchCheckinConfigs } from "@gacha-expo-app/domain/user/action/checkin.action";
import { clearOperationStatus } from "@gacha-expo-app/domain/user/store";
import { OripaCard } from "@gacha-expo-app/components/common/oripa-card";
import { StampCard } from "@gacha-expo-app/components/stamp-card/stamp-card";
import { user } from "@commons/domain";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (fetchCheckinConfigsOperation.status !== 'succeeded') {
      return ;
    }
    if (fetchCheckinConfigsOperation.payload?.length && fetchCheckinConfigsOperation.payload?.length > 0) {
      dispatch(checkin(userProfile?.id));
    }
  }, [userProfile, fetchCheckinConfigsOperation]);
  useEffect(() => {
  ...
  return <div className="w-full lg:max-w-5xl bg-white relative pb-40">
    ...
    <StampCard isOpen={showStampCard} days={stampDays} checkinConfigs={fetchCheckinConfigsOperation.payload} onConfirm={() => setShowStampCard(false)} confirmLabel={"確認"} />
  </div>
}
