...
import { CardEdit } from "@gacha-expo-app/components/common/card-edit";
  ...
  const onSubmit = (data: CardForm) => {
    if (updateCardOperation.status === 'confirm' && updateCardOperation.payload) {
      dispatch(updateCardById({
        ...
        card: { ...data, collectionId: params.id, status: data.status ? 'active' : 'inactive' }
      })).unwrap().then(() => {
