...
import { useAppDispatch } from "@gacha-expo-app/store/hooks";
import { useRouter } from "next/navigation";
import { LoginResponse, clearOperationStatus, fetchUserProfile, sendVerificationEmail, verifyEmail } from "@gacha-expo-app/domain/user";
...
export default function VerifyEmailPage({
  ...
  return isVerified ? 
    ...
    <div className="max-w-xl p-8 text-center text-gray-800 bg-white  lg:max-w-3xl rounded-3xl lg:p-12">
      ...
      <div className="mt-4">
      <PrimaryButton className="bg-primary shadow-none hover:shadow-none" onClick={() => router.push("/")}>ホームページに戻る</PrimaryButton>
      </div>
    </div>
  ...
  <div className="max-w-xl p-8 text-center text-gray-800 bg-white lg:max-w-3xl rounded-3xl lg:p-12">
    ...
    <div className="mt-4">
      <PrimaryButton className="bg-primary shadow-none hover:shadow-none" onClick={() => dispatch(clearOperationStatus('sendResetPasswordEmail')).then(() => dispatch(sendVerificationEmail(params.id)))}>検証メール再送信</PrimaryButton>
