import {  LoginResponse } from '@gacha-expo-app/domain/user/dto/login.dto';
...
export interface UserState {
  ...
  operations: {
    ...
    fetchCheckinConfigs: types.Operation<CheckinConfig[]>
  }
  myInfo: {
    userProfile?: User;
...
export const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchCheckinConfigs.fulfilled, (state, { payload }) => {
      state.operations.fetchCheckinConfigs = {
        status: 'succeeded',
        payload,
      }
    });
