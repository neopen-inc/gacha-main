...
import { CollectionProgress } from "@gacha-nango-app/domain/types";
...
export function DisplayOripa({ oripa , collectionProgress, onClick}: DisplayOripaProps ) {
  return <Card 
    className={`${Number(collectionProgress?.inventory) === 0 ? 'grayscale' : ''} shadow-md cursor-pointer overflow-hidden aspect-[5/4] w-[300px] m-[5px] shadow-all `}
    onClick={onClick}
  >
    <div>
      <div className="group relative overflow-hidden flex flex-col p-2">
          <img
            className="mx-auto rounded-lg transition duration-500 group-hover:scale-105 overflow-hidden w-full aspect-[3/2] object-top"
            src={oripa.thumbnail}
            alt={oripa.name}
            loading="lazy"
          />
      </div>
      <div className="m-0 px-2 bg-transparent rounded-none cursor-pointer z-10 transform -translate-y-1/2" >
          <div className="inset-x-0 h-20 mt-auto bg-transparent dark:bg-white transition duration-300 ease-in-out  z-10">
            {
