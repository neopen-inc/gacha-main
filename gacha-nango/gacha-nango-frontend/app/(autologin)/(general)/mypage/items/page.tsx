...
import { fetchUserProfile } from "@gacha-nango-app/domain/user";
import { ResultBox } from "@gacha-nango-app/components/common/result-box";
import { setPageTitle, setReturnPage } from "@gacha-nango-app/store/page";
import store from "@gacha-nango-app/store";
import _ from "lodash";
import { BaseDialog } from "@gacha-nango-app/components/dialog/base-dialog";
import { LineItem } from "@gacha-nango-app/domain/oripa/types/line-item";
import { clearReturnItem, fetchUserLineItems, prepareReturnItem, returnMany } from "@gacha-nango-app/domain/oripa/action";
import { clearCreateShipping, createShipping, prepareCreateShipping } from "@gacha-nango-app/domain/shipping/action/shipping.action";
...
export default function MyLineItems() {
  ...
  const renderCards = (status: string, selectable: boolean) => {
    ...
    return <div className="grid gap-y-5 gap-x-2 lg:gap-x-6 grid-cols-2 lg:grid-cols-2 w-full pt-5">
      {
        lineItems && lineItems.data.map((lineItem, index) =>
          <ResultBox key={index} thumbnail={lineItem.card.thumbnail} name={lineItem.card.name} grade={lineItem.card.grade} points={lineItem.card.points.toString()} selected={!!selected[lineItem.id]} onClick={() => { selectable && toggleSelect(lineItem.id) }} />)
      }
    </div>
  }
     ...
     variant="fullWidth" className="w-full" onChange={handleChange} aria-label="">
      {data.map(({ label, value }) => (
          <Tab
            ...
            style={{color: activeTab === value ? "white" : "#00959a"}}
        ...
        activeTab === 'unselected' &&
          <footer className="w-full h-30 border-t-2 border-gray-300 pt-5 mt-10">
            <div className="lg:space-x-10 h-30 flex flex-col space-y-2  lg:p-5" >
              <div className="grid grid-cols-1 lg:gap-x-6 gap-y-5 md:grid-cols-2 ">
