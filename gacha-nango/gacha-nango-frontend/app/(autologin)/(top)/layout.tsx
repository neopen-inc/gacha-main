...
import { fetchUserProfile, setAuthInfo } from "@gacha-nango-app/domain/user/store";
import React from "react";
import { numberUtils } from "@common-utils";
import { Button, Typography } from "@mui/material";
import { UserPointBadge } from "@gacha-nango-app/components/user-point-badge";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <div >
    <div className="bg-primary text-white text-sm flex flex-row justify-between leading-8 align-middle items-baseline lg:px-10 px-2">
      <div><span className="font-extralight text-xs ">トレーディングカードのガチャ　EC専門サイト</span>　<span className="font-semibold hidden lg:inline-block">ガチャガチャ</span></div>
    </div>
        ...
        <div className="flex items-center justify-between text-blue-gray-900">
          <div className="flex justify-start items-center">
            <Link className="md:mr-8 mr-2" href="/">
              <Typography
                variant="h6"
                className="cursor-pointer py-1 font-lg text-primary"
              >
                ガチャガチャ
              </Typography>
            </Link>
