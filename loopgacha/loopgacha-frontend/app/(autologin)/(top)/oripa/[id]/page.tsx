...
import { fetchCategories } from "@loopgacha-app/domain/oripa/action/category.action";
import { fetchCollectionById, fetchCollectionProgress, fetchCollections } from "@loopgacha-app/domain/oripa/action/collection.action";
import { CollectionProgress } from "@loopgacha-app/domain/oripa/types/collection-progress";
import { useAppDispatch, useAppSelector } from "@loopgacha-app/store/hooks";
import { getJSTDateString } from "@loopgacha-app/util/date";
import { useRouter, useSearchParams } from "next/navigation";
import React, { useRef, useState } from "react";
import { useEffect } from "react";
import './oripa.css';
import { numberUtils, types } from "@common-utils";
import { Gacha2Button } from "@loopgacha-app/components/common/button/gacha2-button";
import { createGacha } from "@loopgacha-app/domain/oripa/action/gacha.action";
import { Gacha } from "@loopgacha-app/domain/oripa/types/gacha";
import { fetchSceneByGrade } from "@loopgacha-app/domain/oripa/action/scene.action";
import { isObject, min } from "lodash";
import { fetchUserProfile,fetchCheckinConfigs } from "@loopgacha-app/domain/user";
import { Collection } from "@loopgacha-app/domain/oripa/types/collection";
import { fetchCards } from "@loopgacha-app/domain/oripa/action";
import { Card } from "@loopgacha-app/domain/types";
import { SectionTitle } from "@loopgacha-app/components/common/title/section-title";
...
export default function GachaDetailPage({params}: {
  ...
  useEffect(() => {
    ...
    dispatch(fetchCards({collectionId: params.id, status: 'active', grade: '1', top: 1})).unwrap().then((cards) => {
      setG1Cnt(cards.total)
    });
    dispatch(fetchCards({collectionId: params.id, status: 'active', grade: '2', top: 1})).unwrap().then((cards) => {
  ...
  return <div className="w-full max-w-5xl m-auto relative pb-16 text-center">
        ...
        <img className="mx-auto sm:w-min-96 rounded-lg" src={collection?.thumbnail || ''} alt={collection?.name || ''} />
          <div className="text-center w-[80%] -translate-y-1/2 ">
            ...
            <h4 className="text-xl font-semibold text-white">
              <LinearProgress variant="determinate" value={collectionProgressMap[collection?.id || '']?.progress * 100 || 0} 
              sx={{
                height: '20px'
              }}
              color="progressFull"
          ...
          <div className="max-w-[60%] sm:max-w-[100%] mx-auto grid grid-cols-1 sm:grid-cols-2 gap-2 mt-8">
            ...
            <Gacha2Button disabled={disableGacha} type={"ten"} onClick={() => {
              gacha('ten')
            }} />
          </div>
        </div>
    ...
    <section className="3xl:mx-20 3xl:mt-40 mt-5 md:mt-40 mb-20">
      ...
      <section className="mb-10">
        ...
        {grade1Cards.length < g1Cnt && <div className="text-xs m-5">その他あり</div>}
      </section>
      }
      {
      grade2Cards.length > 0 && <section className="m-10">
        ...
        {grade2Cards.length < g2Cnt && <div className="text-xs m-5">その他あり</div>}
      </section>
      }
      {
