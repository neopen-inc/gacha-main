...
import { fetchUserProfile, setAuthInfo } from "@loopgacha-app/domain/user/store";
import React from "react";
import { Button, Typography } from "@mui/material";
import { UserPointBadge } from "@loopgacha-app/components/user-point-badge";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <div >
    <div className="bg-primary text-white text-sm flex flex-row justify-between leading-8 align-middle items-baseline lg:px-10 px-2">
      <div><span className="font-extralight text-xs ">トレーディングカードのガチャ　EC専門サイト</span>　<span className="font-semibold hidden lg:inline-block">ループガチャ</span></div>
    </div>
