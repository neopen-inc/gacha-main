...
import { fetchUserProfile, setAuthInfo } from "@cardpia-app/domain/user/store";
import React from "react";
import { Button, Typography } from "@mui/material";
import { UserPointBadge } from "@cardpia-app/components/user-point-badge";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <div >
    <div className="bg-primary text-frontPrimary text-sm flex flex-row justify-between leading-8 align-middle items-baseline lg:px-10 px-2">
      <div><span className="font-extralight text-xs ">トレーディングカードのガチャ　EC専門サイト</span>　<span className="font-semibold hidden lg:inline-block">カードピア</span></div>
    </div>
      ...
      <div className="top-0 inset-0 z-10 h-max max-w-full px-2 py-0 rounded-none md:py-2 md:px-4 lg:px-8 lg:py-2 border-b border-[#eee] " >
        <div className="flex items-center justify-between text-blue-gray-900">
          ...
          <div className="flex items-center gap-4">
            ...
            {
              !userProfile && <Button variant="contained" onClick={() => router.push("/login")} >ログイン</Button>
            }
          </div>
        </div>
      </div>
