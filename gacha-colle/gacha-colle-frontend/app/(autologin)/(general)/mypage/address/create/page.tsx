...
import { CustomerInput } from "@gacha-colle-app/components/common/customer-input";
...
import { PostUserAddressDto, createUserAddress, fetchUserAddresses } from "@gacha-colle-app/domain/user";
import { todofukenList } from "@gacha-colle-app/util/todofuken";
...
export default function AddressRegisterPage() {
  ...
  const createAddress = (data: AddressForm) => {
    if (userProfile === undefined) {
      return;
    }
    dispatch(createUserAddress({ userId: userProfile?.id || '', address: data })).then(() => {
      addressForm.reset();
  ...
  return <div>
    <div className="py-6 px-2 lg:px-6">
      <form className="grid grid-cols-2" onSubmit={addressForm.handleSubmit(createAddress)}>
