...
export function OripaCard({ collection, collectionProgress, onClick }: OripaCardProps) {
  return <Card 
    className={`${Number(collectionProgress?.inventory) === 0 ? 'grayscale' : ''} shadow-md cursor-pointer overflow-hidden aspect-[5/4] md:w-max-[360px] w-[calc(100%-10px)] m-[5px] shadow-all`}
    onClick={onClick}
  >
    <div>
