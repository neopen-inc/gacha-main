...
export function SimpleBreadcrumb({ locations }: SimpleBreadcrumbProps) {
  return (
    <div className="flex items-center py-4 overflow-x-auto whitespace-nowrap">
      <Link href={locations[0].link || '#'} className="text-gray-600 dark:text-gray-200">
        <IconHome />
      </Link>
...
<Link href={location.link} className="text-gray-600 dark:text-gray-200 hover:underline">
{location.label}
</Link></div>)
}
    </div>
