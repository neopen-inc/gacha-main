...
import { CustomerInput } from "@nanashinooripa-app/components/common/customer-input";

import { PostUserAddressDto,updateUser,createUserAddress, updateUserAddress, fetchUserProfile  } from "@nanashinooripa-app/domain/user";
...
import { todofukenList } from "@nanashinooripa-app/util/todofuken";
...
export default function AccountPage() {
  ...
  <section className="py-10 w-[80%] flex flex-row flex-wrap">
    <header className="rounded-t-md text-black bg-primary flex flex-row w-full justify-left items-center px-5 py-1">
      <span>アカウント編集</span>
    </header>
      ...
      <form onSubmit={addressForm.handleSubmit(createOrUpdateAddress)}>
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="flex flex-row py-5 space-x-5 justify-center col-span-2">
            <Button type="submit" variant="contained"  className="shrink-0 bg-primary shadow-none hover:shadow-none rounded-full px-16">保存</Button>
          </div>
        </div>
