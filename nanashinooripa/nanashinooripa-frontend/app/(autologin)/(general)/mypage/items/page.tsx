...
import { fetchUserProfile } from "@nanashinooripa-app/domain/user";
import { ResultBox } from "@nanashinooripa-app/components/common/result-box";
import { setPageTitle, setReturnPage } from "@nanashinooripa-app/store/page";
import store from "@nanashinooripa-app/store";
import _ from "lodash";
import { BaseDialog } from "@nanashinooripa-app/components/dialog/base-dialog";
import { LineItem } from "@nanashinooripa-app/domain/oripa/types/line-item";
import { clearReturnItem, fetchUserLineItems, prepareReturnItem, returnMany } from "@nanashinooripa-app/domain/oripa/action";
import { clearCreateShipping, createShipping, prepareCreateShipping } from "@nanashinooripa-app/domain/shipping/action/shipping.action";
     ...
     variant="fullWidth" className="w-full" onChange={handleChange} aria-label="">
      {data.map(({ label, value }) => (
          <Tab
            ...
            style={{color: activeTab === value ? "black" : "black", background: activeTab === value ? "#FFF3B8" : "#f5f5f5"}}
            className={`${activeTab === value ? "md:font-bold bg-primary font-semibold " : "md:font-semibold bg-primary-200"} px-0 py-1 md:py-2 text-sm md:mx-2 md:rounded-md`}
          />
        ))}
      </TabList>
          ...
          <footer className="w-full h-30 border-t-2 border-gray-300 pt-5 mt-10">
            <div className="lg:space-x-10 h-30 flex flex-col space-y-2 lg:p-5" >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 m-2">
                ...
                <Button variant="contained" className="basis-0 col-span-2 py-2 shadow-none hover:shadow-none rounded-full" onClick={onCreateReturn}>{selectedCount() === 0 ? 'すべてのカードを還元' : `${selectedCount()}枚カードを還元`}</Button>
                <Button disabled={!hasSelectedItems()} variant="contained" className="basis-0 col-span-2 py-2 bg-primary rounded-full" onClick={onCreateShipping}>{selectedCount()}枚発送</Button>
              </div>
            </div>
          </footer>
      }
