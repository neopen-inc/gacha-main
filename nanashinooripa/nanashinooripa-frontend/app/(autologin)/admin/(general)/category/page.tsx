...
import { StatusTag } from "@nanashinooripa-app/components/admin/status-tag";
import { dialog } from "@commons";
import { Pagination } from "@nanashinooripa-app/components/common/pagination";
import { SimpleBreadcrumb } from "@nanashinooripa-app/components/common/simple-breadcrumb";
import { IconDelete } from "@nanashinooripa-app/components/icons/delete";
import { IconPencil } from "@nanashinooripa-app/components/icons/pencil";
import { useAppDispatch, useAppSelector } from "@nanashinooripa-app/store/hooks";
import { Button, FormControlLabel, Switch, TextField } from "@mui/material";
import _ from "lodash";
import React, { useEffect, useMemo } from "react";
import { Controller, useForm } from "react-hook-form";
import { clearCreateCategory, clearRemoveCategory, clearUpdateCategory, createCategory, fetchCategories, prepareCreateCategory, prepareRemoveCategory, prepareUpdateCategory, removeCategoryById, updateCategoryById } from "@nanashinooripa-app/domain/oripa/action";
import { PostCategoryDto } from "@nanashinooripa-app/domain/oripa/dto";
import FileUploader from "@nanashinooripa-app/components/common/file-uploader";
...
export default function AdminCategoryPage() {
  ...
  const category = useAppSelector(state => state.oripa.category.category);
  ...
  const confirmRemoveCategory = () => {
    ...
    dispatch(removeCategoryById(category.id)).unwrap().then(() => {
  ...
  const onSubmit = (data: CategoryForm) => {
    if (updateCategoryOperation.status === 'confirm' && updateCategoryOperation.payload) {
      dispatch(updateCategoryById({ id: updateCategoryOperation.payload.id, 
        category: { ...data,  status: data.status ? 'active' : 'inactive' } })).unwrap().then(() => {
          dispatch(notification.showNotification({
  ...
  const closeEditOrCreateDialog = () => {
    ...
    if (createCategoryOperation.status === 'confirm') {
      //console.log("clearCreateCategory");
      dispatch(clearCreateCategory());
    }
  }
  ...
  return <>
    ...
    <dialog.ConfirmDialog shouldOpen={removeCategoryOperation.status === 'confirm'}
     ...
     title="カテゴリー削除"
    >
      {category && category.name}を削除しますか？
