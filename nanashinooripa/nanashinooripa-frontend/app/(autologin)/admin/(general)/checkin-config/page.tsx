...
import { CheckinConfig, removeCheckinConfig } from "@nanashinooripa-app/domain/user";
import { useAppDispatch, useAppSelector } from "@nanashinooripa-app/store/hooks";
import { clearOperationStatus, confirmCreateCheckinConfig, confirmDeleteCheckinConfig, confirmUpdateCheckinConfig, createCheckinConfig, fetchCheckinConfigs, getCheckinConfigs, updateCheckinConfig } from "@nanashinooripa-app/domain/user";
      ...
      <div className="p-4 bg-white rounded">
        ...
        <table className="w-full mt-2 text-gray-500">
          ...
          <tbody className="divide-y divide-gray-200">
            {sortBy(checkinConfigs, 'days').map((checkinConfig, index) => <tr key={index} className="cursor-pointer hover:bg-slate-100">
    ...
    <dialog.FormDialog open={createCheckinConfigOperation.status === 'confirm' || updateCheckinConfigOperation.status === 'confirm'} 
      ...
      <div className="space-y-4">
        <Controller
          ...
          render={({ field }) => <TextField label={"日数"} {...field} placeholder="日数" />}
        />
        <Controller
          name="points"
          control={checkinForm.control}
          render={({ field }) => <TextField label={"ポイント"} {...field} placeholder="ポイント" />}
        />
        <Controller
          name="clearPoints"
          control={checkinForm.control}
          render={({ field }) => <FormControlLabel label="集計をリセット" control={<Switch checked={field.value} onChange={field.onChange} />}  /> }
        />
      </div>
    </dialog.FormDialog>
