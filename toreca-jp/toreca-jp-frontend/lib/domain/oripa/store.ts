...
import { createCard, fetchCardById, fetchCards, prepareCreateCard, prepareRemoveCard, prepareUpdateCard, removeCardById, updateCardById } from "./action/card.action";
...
import { fetchUserLineItems, prepareCreateShipping, prepareReturnItem, returnItem, returnMany, updateShipping, waitForShipGroups } from "./action/line-item";
...
import { clearRandomlizeCollection, clearResetInventory, createCardToOripa, fetchCardToOripaByCollection, fetchCollectionInitializeStatus, prepareCreateCardToOripa, prepareRandomlizeCollection, prepareRemoveCardToOripa, prepareResetInventory, prepareUpdateCardToOripa, randomlizeCollection, removeCardToOripaById, resetInventory } from "./action/card-to-oripa.action";
import { CardToOripa } from "./types/card-to-oripa";
import { CollectionCardCount } from "./dto/card-to-oripa";
import { createShipping } from "../shipping/action/shipping.action";
...
type OperationType = 'createCategory'
  ...
  | 'updateCard' | 'removeCategory' | 'removeCollection' | 'removeCard' | 'returnItem' | 'createShipping'
...
export interface OripaState {
  ...
  operations: {
    createCategory: Operation<Category>,
    updateCategory: Operation<Category>,
    removeCategory: Operation<Category>,
    fetchCategories: Operation<types.Paginated<Category>>,
    fetchCategoryById: Operation<Category>,

    removeCollection: Operation<Collection>,
    createCollection: Operation<Collection>,
    updateCollection: Operation<Collection>,
    fetchCollections: Operation<types.Paginated<Collection>>,
    fetchCollectionById: Operation<Collection>,
    fetchCollectionProgress: Operation<CollectionProgress[]>,
    
    createCard: Operation<Card>,
    updateCard: Operation<Card>,
    removeCard: Operation<Card>,
    fetchCards: Operation<types.Paginated<Card>>,
    fetchCardById: Operation<Card>,
    
    createScene: Operation<Scene>,
    removeScene: Operation<Scene>,
    updateScene: Operation<Scene>,
    fetchScenes: Operation<Scene[]>,
    fetchSceneByGrade: Operation<Scene>,
    
    updateShipping: Operation<Shipping>,
    createShipping: Operation<PostShippingDto>
    returnItem: Operation<Omit<PostShippingDto, 'addressId'>>

    fetchCardToOripaByCollection: Operation<types.Paginated<CardToOripa>>,
    createCardToOripa: Operation<CardToOripa>,
    removeCardToOripaById: Operation<CardToOripa>,
    updateCardToOripaById: Operation<CardToOripa>,
    randomlizeCollection: Operation<Collection>,
    resetCollection: Operation<Collection>,
    fetchCollectionInitializeStatus: Operation<CollectionCardCount[]>,
  }
}

/*
    upload: Operation<{ url: string }>,
    gacha: Operation<Gacha>,*/
...
const initialState: OripaState = {
  ...
  operations: {
    ...
    fetchCardToOripaByCollection: {
      status: 'idle'
    },
    createCardToOripa: {
      status: 'idle'
    },
    updateCardToOripaById: {
      status: 'idle'
    },
    removeCardToOripaById: {
      status: 'idle'
    },
...
export const oripaSlice = createSlice({
  ...
  }).addCase(clearCreateScene.fulfilled, (state, { payload }) => {
    state.operations.createScene = {
      status: 'idle',
    }
  });
  ...
  }).addCase(clearRemoveScene.fulfilled, (state, { payload }) => {
    state.operations.removeScene = {
      status: 'idle',
    }
  });
  ...
  }).addCase(clearUpdateScene.fulfilled, (state, { payload }) => {
    state.operations.updateScene = {
      status: 'idle',
    }
  });
  ...
  builder.addCase(fetchCardToOripaByCollection.fulfilled, (state, { payload }) => {
    state.operations.fetchCardToOripaByCollection = {
      status: 'succeeded',
      payload,
    };
  });
  builder.addCase(createCardToOripa.fulfilled, (state, { payload }) => {
    state.operations.createCardToOripa = {
      status: 'succeeded',
      payload,
    };
  });
  builder.addCase(removeCardToOripaById.fulfilled, (state, { payload }) => {
    state.operations.removeCardToOripaById = {
      status: 'succeeded',
    };
  }).addCase(removeCardToOripaById.rejected, (state, { payload }) => {
    state.operations.removeCardToOripaById = {
      status: 'failed',
      message: '削除に失敗しました。'
    };
  });
  builder.addCase(prepareCreateCardToOripa.fulfilled, (state, { payload }) => {
    state.operations.createCardToOripa = {
      status: 'confirm',
    };
  }).addCase(prepareRemoveCardToOripa.fulfilled, (state, { payload }) => {
    state.operations.removeCardToOripaById = {
      status: 'confirm',
      payload,
    };
  }).addCase(prepareUpdateCardToOripa.fulfilled, (state, { payload }) => {
    state.operations.updateCardToOripaById = {
      status: 'confirm',
      payload,
    };
  });
  ...
  builder
    ...
    builder.addCase(prepareCreateShipping.fulfilled, (state, action) => {
