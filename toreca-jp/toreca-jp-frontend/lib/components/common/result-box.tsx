...
export function ResultBox({ thumbnail, name, grade, points, selected, rarity, onClick }: ResultBoxProps) {
  return <div className={`flex-row  max-h-[12rem] md:max-w-[90%] max-w-[100%] justify-between rounded shadow-md  ${selected ? 'bg-gray-300' : ''}`} onClick={onClick}>
    <div className="flex justify-between  h-full">
      <div className="max-w-[30%]  h-full relative flex-none rounded-xl bg-transparent -rotate-6">
        <img src={thumbnail} alt="image" className="object-cover w-full" />
      </div>
      <div className="w-3/5 p-2 lg:pr-7 shrink-0 rounded-r-none relative flex flex-col items-end text-right md:p-5 md:space-5">
        ...
        {grade ? <div className="text-xs md:font-semibold">{grade}</div> : <></>}
        <div className="text-xs md:font-semibold">{rarity}</div>
