...
import { BanknotesIcon, EnvelopeIcon, EnvelopeOpenIcon, FilmIcon, GiftTopIcon, RectangleGroupIcon, RectangleStackIcon, SparklesIcon, TruckIcon, UsersIcon } from "@heroicons/react/24/solid";
...
export function AdminSidebar() {
  ...
  return <aside className="hidden w-64 bg-gray-800 sm:block">
    <div className="py-3 text-2xl uppercase text-center tracking-widest bg-gray-900 border-b-2 border-gray-800 mb-8">
      <a href="/" className="text-white">トレカジャパン</a>
    </div>
    <nav className="text-sm text-gray-300">
      <ul className="flex flex-col">
        ...
        <li className={`px-4 cursor-pointer ${isExactActive ? 'bg-gray-500 text-gray-800' : 'hover:bg-gray-700'}`}>
          <a className="py-3 flex items-center gap-2" href="/admin/card">
            <RectangleStackIcon className="w-5 h-5 gap-2" />
            カード管理
          </a>
        </li>
        <li className={`px-4 cursor-pointer ${isExactActive ? 'bg-gray-500 text-gray-800' : 'hover:bg-gray-700'}`}>
...
/**
 <li className="px-4 cursor-pointer hover:bg-gray-700">
        ...
        <li className={`px-4 cursor-pointer ${isExactActive ? 'bg-gray-500 text-gray-800' : 'hover:bg-gray-700'}`}>
          <a className="py-3 flex items-center gap-2" href="/admin/carousel">
