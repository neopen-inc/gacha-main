...
import { fetchShippings } from "@toreca-jp-app/domain/shipping/action/shipping.action";
...
export default function AdminShippingPage({ searchParams }: { searchParams: { status: string } }) {
  ...
  React.useEffect(() => {
    dispatch(fetchShippings({ status: (searchParams.status as ('waiting' | 'shipped')) || searchParamHooks.get('status') || 'waiting', orderby: 'createdAt desc' }));
  }, []);
      ...
      <div className="p-4 bg-white rounded">
        ...
        <table className="w-full mt-2 text-gray-500 text-sm">
          ...
          <tbody className="divide-y divide-gray-200">
            {fetchShippingsOperation.payload?.data.map((shipping: Shipping, index: number) => <tr key={index} onClick={() => { router.push(`/admin/shipping/${shipping.id}`) }} className="cursor-pointer hover:bg-slate-100">
          ...
          <tfoot>
            <tr>
              ...
              <Pagination total={fetchShippingsOperation.payload?.total || 0} count={fetchShippingsOperation.payload?.count || 0} offset={fetchShippingsOperation.payload?.offset || 0} limit={fetchShippingsOperation.payload?.limit || 0} onPage={function (page: number, limit: number): void {
                dispatch(fetchShippings({ status: (searchParams.status as ('waiting' | 'shipped')) || 'waiting', skip: (page - 1) * limit, top: limit, orderby: 'createdAt DESC' }))
              }} />
              </td>
